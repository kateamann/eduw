 ...
 * Edinburgh Unwrapped.
 *
 * This file adds the required WooCommerce setup functions to the Edinburgh Unwrapped Theme.
 *
...
add_action( 'wp_enqueue_scripts', 'eduw_products_match_height', 99 );
/**
 * Prints an inline script to the footer to keep products the same height.
 *
 * @since 2.3.0
 */
function eduw_products_match_height() {
...
add_filter( 'woocommerce_style_smallscreen_breakpoint', 'eduw_woocommerce_breakpoint' );
/**
 ...
 */
function eduw_woocommerce_breakpoint() {
...
add_filter( 'genesiswooc_products_per_page', 'eduw_default_products_per_page' );
/**
 ...
 */
function eduw_default_products_per_page() {

	return 8;

}

add_filter( 'woocommerce_pagination_args', 'eduw_woocommerce_pagination' );
/**
 ...
 */
function eduw_woocommerce_pagination( $args ) {

	$args['prev_text'] = sprintf( '&laquo; %s', __( 'Previous Page', 'eduw' ) );
	$args['next_text'] = sprintf( '%s &raquo;', __( 'Next Page', 'eduw' ) );

	return $args;

}

add_action( 'after_switch_theme', 'eduw_woocommerce_image_dimensions_after_theme_setup', 1 );
/**
 * Defines WooCommerce image sizes on theme activation.
 *
 * @since 2.3.0
 */
function eduw_woocommerce_image_dimensions_after_theme_setup() {
	...
	eduw_update_woocommerce_image_dimensions();

}

add_action( 'activated_plugin', 'eduw_woocommerce_image_dimensions_after_woo_activation', 10, 2 );
/**
 ...
 */
function eduw_woocommerce_image_dimensions_after_woo_activation( $plugin ) {
 ...
 * @since 2.3.0
 */
function eduw_update_woocommerce_image_dimensions() {
...
add_filter( 'woocommerce_get_image_size_gallery_thumbnail', 'eduw_gallery_image_thumbnail' );
/**
 ...
 */
function eduw_gallery_image_thumbnail( $size ) {
